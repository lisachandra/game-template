...
local APIDump
do
	APIDump = script:GetAttribute("APIDump")
	if APIDump then
		APIDump = HttpService:JSONDecode(APIDump)
	else
		local success
		success, APIDump = require(script.Parent:FindFirstChild("APIDump")):await()
		if not success then
		...
		script:SetAttribute("APIDump", HttpService:JSONEncode(APIDump))
	end
end
