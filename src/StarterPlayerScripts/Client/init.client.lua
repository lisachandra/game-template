...
local Player = Players.LocalPlayer; if not Player.Character then
...
local function merge(a, b, none)
    local new = table.clone(a); for k, v in b do
...
for _i, system in ipairs(systems) do
    local success, err = Promise.try(system.run):await(); if not success then
